...
class Suggestion(Model):
    suggester = fields.ForeignKeyField(
        'models.User', on_delete=fields.RESTRICT, related_name='suggester', null=True)
    reviewer = fields.ForeignKeyField(
        'models.User', on_delete=fields.RESTRICT, related_name='suggestion_reviewer', null=True)
...
class IntervalRecognitionLevel(Model):
    ...
    performance = fields.ForeignKeyField(
        'models.LevelPerformance', on_delete=fields.RESTRICT, null=True)
...
class ChordRecognitionLevel(Model):
    ...
    shape = fields.CharField(max_length=100, validators=[
                             validators.CommaSeparatedIntegerListValidator(allow_negative=False)])
...
class ModeRecognitionLevel(Model):
    ...
    shape = fields.CharField(max_length=100, validators=[
                             validators.CommaSeparatedIntegerListValidator(allow_negative=False)])
    characteristic_notes = fields.CharField(max_length=100, validators=[
                                            validators.CommaSeparatedIntegerListValidator(allow_negative=False)])
...
class MelodicDictationLevel(Model):
    ...
    time_signature = fields.CharField(
        max_length=100, default="4/4", validators=time_signature_validator)
    performance = fields.ForeignKeyField(
        'models.LevelPerformance', on_delete=fields.RESTRICT, null=True)
...
class FrequencyRecognitionLevel(Model):
    level_number = fields.IntField(unique=True, pk=True)
    description = fields.CharField(max_length=1000)
    frequency_thresholds = fields.CharField(max_length=100, validators=[
                                            validators.CommaSeparatedIntegerListValidator(allow_negative=False)])
    performance = fields.ForeignKeyField(
        'models.LevelPerformance', on_delete=fields.RESTRICT, null=True)
...
class Tune(Model):
    excercise = fields.ManyToManyField(
        'models.Excercise', on_delete=fields.NO_ACTION, through='tune_excercise')
...
class Phrase(Model):
    author = fields.ForeignKeyField(
        'models.User', on_delete=fields.RESTRICT, related_name='author')
    reviewer = fields.ForeignKeyField(
        'models.User', on_delete=fields.RESTRICT, related_name='phrase_reviewer', default=None, null=True)
